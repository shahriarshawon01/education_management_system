...
class DormitoryComponentController extends Controller
{
    use Helper;
    public function __construct()
    {
        $this->model = new DormitoryComponent();
    }

    public function index()
    {
        if (!can('dormitory_component_view')) {
    ...
    {
        if (!can('dormitory_component_add')) {
            return $this->notPermitted();
        }
        $user = auth()->user();
        $authId = auth()->user()->id;
    ...
    {
        if (!can('dormitory_component_update')) {
            return $this->notPermitted();
        }
        try {
            $input = $request->all();
    ...
    {
        if (!can('dormitory_component_delete')) {
            return $this->notPermitted();
        }
        try {
