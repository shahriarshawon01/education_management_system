...
use App\Models\Website\Facultie;
...
class FacultieController extends Controller
{
    ...
    {
        $this->model = new Facultie();
    }

    public function index()
    {
        if (!can('faculties_view')) {
        ...
        try {
            ...
            $data = $this->model 
            ->with('departments:id,department_name','teachers:id,name')
            ->when($keyword, function ($query) use ($keyword) {
                $query->orWhereHas('departments', function ($q) use ($keyword) {
                    $q->where('department_name', 'like', "%$keyword%");
                });
            })
            ->when($keyword, function ($query) use ($keyword) {
                $query->orWhereHas('teachers', function ($q) use ($keyword) {
