...
<template :key="$route.meta.name">
  <div class="main_component">
    <div
      ...
      tabindex="-1"
      aria-labelledby="certificate_modalLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="certificate_modal">
              Academic Document
            </h1>
            <button
              type="button"
      ...
      <div class="col">
        ...
        >
          <template v-slot:page_top>
            <page-top
              topPageTitle="Merit List"
              :default-add-button="false"
              page-modal-title="Admission Add/Edit"
            >
              <div class="btn">
                <select
                  @change="finalMultipleStudentEnroll($event.target.value)"
                  class="form-control">
              ...
              <button @click="printData('printDiv')" class="btn btn-primary btn-sm">
                <i class="fa fa-print"></i> Print
              </button>
              <a class="btn btn-success btn-sm"
                       @click="openFile(`${urlGenerate('api/applicant_merit_export_excel')}`)"><i class="fa fa-file-excel"></i><i class="fa fa-file-excel-o"></i> Export Excel</a>
            </page-top>
          </template>
              ...
              <th class="d-print-none">
                <div class="form-check">
                  <input
                    ...
                    v-model="checked"
                    id="selectAll"
                    @click="selectAllCheck($event, dataList.data)"
                  />
                </div>
              </th>
              <th>SL</th>
            ...
            <tr v-for="(data, index) in dataList.data">
              <td class="d-print-none">
                <div
                  ...
                  class="form-check"
                  v-if="data.final_enroll_status == '1'"
                >
                  <input
                    class="form-check-input"
                    type="checkbox"
              ...
              <!-- <td>{{showData(data.shift, 'name')}}</td> -->
              <td>
              ...
              <td>
                <span
                  v-if="data.short_list == 2"
                  v-for="(result, result_index) in data.previous_result"
                >
                  <div class="badge bg-dark">{{ result.exam_type }}</div>
              ...
              <td >
                ...
                <span
                  v-else-if="data.final_enroll_status == '0'"
              ...
              >
                <div class="hstack gap-1 fs-15 no-print">
                  ...
                  >
                    <button
                      class="btn btn-sm btn-light disabled-link"
                      :disabled="true"
                    >
                  ...
                  <a :href="'/api/view_application_payment_details'+'/'+data.id" target="_blank" v-if="data.final_enroll_status == '1'">
                    <button class="btn btn-sm btn-light">
                        <i class="fa fa-eye"></i>
                    </button>
                  </a>

                  <a class="link-danger" @click="custom_merit_status(data.id)"
                    ><button class="btn btn-sm btn-light">
  ...
  <!-- <generalModal
      ...
      :isEnablePrint="false"
      :isEnableSubmitForInvoice="false"
      @submit="submitStudentForAdmission()"
    >
          ...
          <div class="row form-group mt-3">
            <div class="col-4">
              <label class="control-label"
                >{{ fees_type.name }} ({{ fees_type.amount }})</label
              >
            </div>
            <div class="col-2">
              <label>
                <input
                  @change="checkAmount($event, fees_type)"
                  type="checkbox"
                  :checked="fees_type.checked"
                />
              </label>
            </div>
            <div class="col-6">
 ...
 <formModal modalSize="modal-xs" modal-id="detailsModal" @submit="submitStudentForAdmission()">
      <template>
        <template v-for="(fees_type, index) in formObject.fees">
          <div class="row form-group mt-3">
            <div class="col-4">
...
export default {
  ...
  components: { PageTop, DataTable, generalModal,formModal },
  data() {
    return {
  ...
  methods: {
    ...
    submitStudentForAdmission() {
      const _this = this;
      axios.post("/api/student_store_from_admission", _this.formObject)
        .then((res) => {
          console.log(res)
          _this.getDataList();
          _this.$toastr(
    ...
    custom_merit_status(id) {
      axios
        .get("/api/custom_merit_status/" + id)
        .then((res) => {
          this.getDataList();
    ...
    viewEnroll: function (data) {
      ...
      _this.$set(_this.formObject, "session_year_id", data.session_year_id);
      _this.$set(_this.formObject, "section_id", data.section_id);
       _this.$set(_this.formObject, "department_id", data.department_id);
  ...
  mounted() {
    ...
    _this.getDataList();
    _this.getGeneralData(["class", "session", "section", "departments"]);
  },
};
</script>
