<template :key="$route.meta.name">
  <div class="main_component">
    <div class="row">
      <div class="col">
        <data-table :table-heading="tableHeading" table-title="Canteen Invoice"
          :default-object="{ month: '', invoice_category: '' }">
          <template v-slot:page_top>
            <page-top topPageTitle="Canteen Invoice" page-modal-title="Canteen Invoice" :default-add-button="false">
            </page-top>
          </template>
          <template v-slot:filter>
            ...
            <div class="col-md-2">
              <select class="form-control" v-model="formFilter.invoice_category" name="invoice_category">
            ...
            <tr v-for="(data, index) in dataList.data">
              ...
              <template v-if="data.invoice_type == '1'">
                <td>
                  ...
                  <strong>Institute : </strong>{{ data ? data.school_name : "" }}
                </td>
              </template>
        ...
        <div id="printInvoice" class="invoice-container" v-if="invoiceDetails && invoiceDetails.invoice">
          <div class="invoice-header mb-3 p-3 border rounded shadow-sm bg-light">
            <div>
              ...
              <span><strong>Invoice Type:</strong> {{ getInvoiceType(invoiceDetails.invoice.invoice_type)
                }}</span><br />
            ...
            <template v-if="invoiceDetails.invoice.invoice_type == 1">
              <hr />
              <span><strong>Name:</strong> {{ invoiceDetails.invoice.student_name_en }}</span><br />
              <span><strong>Session:</strong> {{ invoiceDetails.invoice.session_title }}</span><br />
              <span><strong>ID:</strong> {{ invoiceDetails.invoice.student_roll }}</span><br />
              <span><strong>Class:</strong> {{ invoiceDetails.invoice.class_name }}</span><br />
              
            </template>

            <template v-else-if="invoiceDetails.invoice.invoice_type == 2">
              <hr />
              <span><strong>Name:</strong> {{ invoiceDetails.invoice.employee_name }}</span><br />
              <span><strong>Department:</strong> {{ invoiceDetails.invoice.department_name }}</span><br />
              <span><strong>Designation:</strong> {{ invoiceDetails.invoice.designation_name }}</span><br />
...
export default {
  ...
  data() {
    return {
      tableHeading: ["Sl", "Generate", "Invoice", "Student/Employee Info", "Created By", "Total Amount", "Payable", "Action",],
      invoiceDetails: [],
      years: [],
    };
  },
