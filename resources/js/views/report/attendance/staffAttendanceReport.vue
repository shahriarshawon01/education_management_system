        ...
        <div class="row">
            <div class="col">
                <data-table :defaultSearchButton="false" :table-heading="[]"
                    ...
                    :defaultFilter="false" :default-pagination="false" table-title="Staff Attendance Report"
                    :table-responsive="false">

                    <template v-slot:page_top>
                        <page-top :default-object="{}" topPageTitle="Staff Attendance Report"
                    ...
                    <template v-slot:filter>
                        ...
                        <div class="col-md-2">
                            <button class="btn btn-primary" @click.prevent.stop="getStaffAttendance">Get
                                Attendance</button>
                        </div>
                    </template>
                    ...
                    <template v-slot:bottom_data>
                        <form @submit.prevent>
                            <template v-if="combinedEmployeeData.staffs && combinedEmployeeData.staffs.length > 0">

                                <!-- Reusable School Info Component -->
                                <schoolInfo title="Staff Attendance Report" />
                                
                                <div class="attendance-info">
                                    <span class="attendance-label"><strong>Reporting Date :</strong> {{ reportingDateRange }}</span>
                                </div>
                                    ...
                                    <table class="table table-bordered">
                                        <thead>
                                            <tr>
                                                ...
                                                <th rowspan="2" class="sticky-column">Staff</th>
                                        ...
                                        <tbody>
                                            <tr v-for="(staff, index) in combinedEmployeeData.staffs" :key="staff.id">
                                                ...
                                                <template v-for="(dateT, dateIndex) in attendanceDates">
                                                    ...
                                                    <td v-else-if="staff.checkInStatus[dateT.date_formatted]">
                                                        <div>
                                                            <span style="font-weight: bold;"
                                                                :style="{ color: getStatusColor(staff.checkInStatus[dateT.date_formatted].presentStatus) }">
                                                                {{
                                                                    staff.checkInStatus[dateT.date_formatted].presentStatus.charAt(0)
                                                                }}
                                                            </span><br>
                                                            <span
                                                                v-if="staff.checkInStatus[dateT.date_formatted].check_in"
                                                                :style="{ color: getStatusColor(staff.checkInStatus[dateT.date_formatted].presentStatus) }">
                                                                CI : {{
                                                                    formatTime(staff.checkInStatus[dateT.date_formatted].check_in)
                                                                }}
                                                            </span> <br>
                                                            <span
                                                                v-if="staff.checkInStatus[dateT.date_formatted].check_out"
                                                                :style="{ color: getStatusColor(staff.checkInStatus[dateT.date_formatted].presentStatus) }">
                                                                CO : {{
                                                                    formatTime(staff.checkInStatus[dateT.date_formatted].check_out)
                                                                }}
                                                            </span>
                                                        </div>
                                                    </td>
...
export default {
    ...
    methods: {
        ...
        getStaffAttendance() {
            ...
            this.axios.get('/api/reports/staff_attendance', {
                ...
                if (Array.isArray(this.combinedEmployeeData.staffs) && this.combinedEmployeeData.staffs.length === 0) {
        ...
        exportToExcel() {
            const headers = ["SL", "Staff", ...this.attendanceDates.map(date => date.toFormat("dd"))];

            const data = this.combinedEmployeeData.staffs.map((staff, index) => {
                const row = [index + 1, `${staff.name} (${staff.id})`];

                this.attendanceDates.forEach(date => {
                    const status = staff.checkInStatus[date.date_formatted];
